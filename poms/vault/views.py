...
class VaultViewSet(AbstractViewSet):
    ...
    def health(self, request, *args, **kwargs):
        finmars_vault = FinmarsVault(realm_code=request.realm_code, space_code=request.space_code)
        status_code = finmars_vault.get_health()

        data = {
            "status": "ok",
            "text": "Vault is responded with health check",
            "data": status_code,
        }
    ...
    def get_status(self, request, *args, **kwargs):
        ...
        try:
            ...
            _ = VaultRecord.objects.get(user_code="hashicorp-vault-token")
            finmars_vault = FinmarsVault(realm_code=request.realm_code, space_code=request.space_code)
            data["status"] = "ok"
            data["text"] = "Vault is operational for storing secrets"
            data["data"] = finmars_vault.get_status()
