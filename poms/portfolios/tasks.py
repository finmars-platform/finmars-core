...
def calculate_portfolio_register_record(self, task_id, *args, **kwargs):  # noqa: PLR0912, PLR0915
    """
        ...
        for key, value in transactions_dict.items():
            ...
            for trn in value:
                ...
                for portfolio_register in portfolio_registers:
                    if not portfolio_register.linked_instrument:
                        _l.error(f"{log} portfolio_register={portfolio_register} has no linked_instrument, ignored!")
                        continue
                    ...
                    if record.cash_currency_id == record.valuation_currency_id:
                        ...
                        _l.info(
                            f"{log} valuation_ccy_fx_rate={valuation_ccy_fx_rate} cash_ccy_fx_rate={cash_ccy_fx_rate} "
                        )
                    ...
                    if previous_date_record:
                        ...
                        _l.info(
                            f"{log} len(items)={len(balance_report.items)} "
                            f"nav={nav_previous_register_record_day_valuation_currency}"
                        )
                    ...
                    _l.info(
                        f"{log} len(items)={len(previous_business_day_balance_report.items)} "
                        f"nav={nav_previous_business_day_valuation_currency}"
                    )
...
def calculate_portfolio_register_price_history(self, task_id: int, *args, **kwargs):  # noqa: PLR0912, PLR0915
    """
...
def calculate_portfolio_history(self, task_id: int, *args, **kwargs):  # noqa: PLR0912, PLR0915
    """
    ...
    for d in dates:
        ...
        except PortfolioHistory.DoesNotExist as e:
            if period_type == "daily":
...
def _calculate_group_reconcile_history(day: str, reconcile_group: PortfolioReconcileGroup, task: CeleryTask):
    ...
    if portfolio_reconcile_history.status == PortfolioReconcileHistory.STATUS_OK:
        reconcile_group.last_calculated_at = datetime.now(UTC)
    ...
    err_msg = (
        f"portfolio_reconcile_history {history_user_code} {day} failed due to "
        f"{portfolio_reconcile_history.error_message}"
    )
    _l.error(err_msg)
